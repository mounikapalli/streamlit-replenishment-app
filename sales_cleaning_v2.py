...
def clean_sales_data_pl(pl_df: pl.DataFrame) -> pl.DataFrame:
    ...
    try:
        ...
        # Clean STYLE codes using Polars string operations
        cleaned_df = cleaned_df.with_columns([
        ...
        # Clean store names using Polars string operations
        cleaned_df = cleaned_df.with_columns([
        ...
        try:
            cleaned_df = cleaned_df.with_columns([
                ...
                .cast(pl.Utf8)
                .str.strptime(pl.Date, fmt="%Y-%m-%d %H:%M:%S", strict=False)
                .alias("DATE")
            ])
            ...
            try:
                ...
                st.error(f"❌ Error parsing dates with alternative format: {str(e2)}")
                return pl.DataFrame()
